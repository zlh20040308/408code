...
int findMin(int A[], int n, int B[], int m, int C[], int p) {
    ...
    while (i < n && j < m && k < p && Dmin > 0) {
        // 循环条件是下标ijk还在集合内，并且Dmin≠0
        D = abs(A[i] - B[j]) + abs(B[j] - C[k]) + abs(A[i] - C[k]); // 算下D
        if (D < Dmin) Dmin = D; // 如果D小就更新Dmin
        if (minjg(A[i], B[j], C[k])) i++; // 如果A[i]最小，i++
        else if (minjg(B[j], C[k], A[i])) j++; // 如果B[j]最小，j++
        else k++; // 不然k++
    }
    return Dmin;
}
