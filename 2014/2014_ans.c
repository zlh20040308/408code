...
int WPL(BiTree root) {
    ...
    //如果需要测试基于层次遍历，请把对应注释删除即可(选中多行后control+/或者command+/)
    // return wpl_LevelOrder(root);//基于层次遍历
}

int wpl_PreOrder(BiTree root, int deep) {
    if (root == NULL) return 0; // 空树返回 0
    if (root->lchild == NULL && root->rchild == NULL)
        return deep * root->weight;
    return wpl_PreOrder(root->lchild, deep + 1) + wpl_PreOrder(root->rchild, deep + 1);
}
...
int wpl_LevelOrder(BiTree root) {
    ...
    int end1, end2 = 0; // 队列最多容纳 MaxSize-1 个元素
    ...
    while (end1 != end2) { // 层次遍历，若队列不空则循环
        ...
        if (t->lchild != NULL) { // 若非叶子结点把左结点入队
            q[end2++] = t->lchild;
            newlastNode = t->lchild; // 并设下一层的最后一个结点为该结点的左结点
        }
